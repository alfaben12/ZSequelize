	...
	for (let join_number = 0; join_number < modelJoins.length; join_number++) {
		...
		if (includes !== false) {
			...
			for (let j = 0; j < includes.length; j++) {
				...
				const ModelOne = require('../models/'+ includes[j].fromModel);
				const ModelTwo = require('../models/'+ includes[j].toModel);
				if (includes[j].bridgeType === 'hasMany') {
					ModelOne.hasMany(ModelTwo, {foreignKey:includes[j].toKey})
				}else if (includes[j].bridgeType === 'belongsTo') {
					ModelOne.belongsTo(ModelTwo, {foreignKey:includes[j].fromKey})
				}else{
					ModelOne.hasOne(ModelTwo, {foreignKey:includes[j].toKey})
				}
				
				let where_object = {};
				where_object[includes[j].toKey] = Sequelize.col(includes[j].fromKey);
				
				let required = includes[j].required;
				// let includes = includes[j].includes;
				nested_includes_object['attributes'] = includes[j].attributes[0] == '*' ? {} : includes[j].attributes;
		...
		// return console.log(include_object)
